...
export const scheduleAddHandler = (appointment) => {
  return axios
    .post(`http://localhost:3001/api/users/${Auth.getUserid()}/schedule`, {
      appointment,
    })
    .then((response) => {
      return response;
...
export const scheduleUpdateHandler = (appointment) => {
  return axios
    .put(
      `http://localhost:3001/api/users/${Auth.getUserid()}/schedule/${
        appointment.user_schedule_id
      }`,
      { appointment }
    )
    .then((response) => {
...
export const scheduleDeleteHandler = (appointment_id) => {
  return axios
    .delete(
      `http://localhost:3001/api/users/${Auth.getUserid()}/schedule/${appointment_id}`
    )
    .then((response) => {
      return response;
    })
